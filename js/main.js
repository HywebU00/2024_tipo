...
$(function () {
  ...
  ////////////// 行動版選單切換////////////
  /*-----------------------------------*/
  _body.append('<aside class="sidebar"><div class="m_area"><button type="button" class="sidebarClose">關閉</button></div><div class="menu_overlay"></div></aside>');
  ...
  function hideSidebar() {
    ...
    // if ($(window).outerWidth() < wwMedium) {
    //   $('.sidebarCtrl.tabletUse').focus();
    // } else {
  ...
  $('.header .menu a,.header .menu button')
    ...
    .on('keydown', function (e) {
      if (e.which === 9 && e.shiftKey) {
        e.preventDefault();
        $('.header .menu .menuClose').focus();
      }
    });

  // -------------------------------------------- subquick選單動作
  let quickLinkCheck;
  let mobileQuickLinkCheck;

  $('.quickLink button').on('click', function (e) {
    let thisClass = $(this).attr('class');
    quickLinkCheck = $(this).parent('li').index();
    $(`.subquick.${thisClass}`).fadeIn('fast');
    $(`.subquick.${thisClass} a,.subquick.${thisClass} button`).eq(0).focus();
  });

  $('.mobileQuickLink button').on('click', function (e) {
    let thisClass = $(this).attr('class');
    mobileQuickLinkCheck = $(this).parent('li').index();
    $(`.subquick.${thisClass}`).fadeIn('fast');
    $(`.subquick.${thisClass} a,.subquick.${thisClass} button`).eq(0).focus();
    $('body').addClass('noscroll');
  });

  $('.subquick .subquickClose').on('click', function (e) {
    $('body').removeClass('noscroll');
    $(`.subquick`).fadeOut('fast');
    if ($(window).outerWidth() < wwMedium) {
      $('.mobileQuickLink button').eq(mobileQuickLinkCheck).focus();
    } else {
      $('.quickLink button').eq(quickLinkCheck).focus();
    }
  });

  $('.subquick').each(function () {
    let _this = $(this);
    let allTarget = _this.find('a,button');
    _this.find('.subquickClose').on('blur', function (e) {
      _this.find('a,button').eq(0).focus();
    });
    allTarget.eq(0).on('keydown', function (e) {
      if (e.which === 9 && e.shiftKey) {
        e.preventDefault();
        _this.find('.subquickClose').focus();
      }
    });
    _this.find('.subquickClose').on('keydown', function (e) {
      if (e.which === 9 && e.shiftKey) {
        e.preventDefault();
        allTarget.eq(allTarget.length - 2).focus();
      }
    });
  });
  // -------------------------------------------- overlay關閉選單
  ...
  $('.searchClose').on('keydown', function (e) {
    let count = $('.m_search a, .m_search button, .m_search input').length;
  ...
  if ($('header .menu').length > 0) {
    var stickyMenuTop = Math.floor($('header .menu').offset().top),
      menuH = Math.floor($('header').outerHeight());
  ...
  $('.sidebar .sidebarClose').on('keydown', function (e) {
    if (e.which === 9 && e.shiftKey) {
      e.preventDefault();
      $('.sidebar a,.sidebar button')
        .eq($('.sidebar a,.sidebar button').length - 2)
        .focus();
    } else if (e.which === 9) {
      e.preventDefault();
      $('.sidebar a,.sidebar button').eq(0).focus();
